...
export default class Battle {
  ...
  getBgmOverride(): string | null {
    ...
    if (globalScene.gameMode.isClassic && this.waveIndex <= 15) {
      return "rhodes1";
    }
    else if (globalScene.gameMode.isClassic && this.waveIndex <= 30) {
      return "rhodes2";
    }
    else if (globalScene.gameMode.isClassic && this.waveIndex <= 45) {
      return "rhodes3";
    }
...
export function getRandomTrainerFunc(
  ...
  return () => {
    ...
    if (trainerConfigs[trainerTypes[rand]].hasDouble && isEvilTeamGrunt) {
      return new Trainer(trainerTypes[rand], TrainerVariant.DOUBLE );
    }
