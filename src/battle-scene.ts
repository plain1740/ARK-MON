...
export default class BattleScene extends SceneBase {
  ...
  launchBattle() {
    ...
    const defaultMoves = [Moves.ARK_01, Moves.ARK_02, Moves.ARK_03, Moves.ARK_04];

    Promise.all([
      Promise.all(loadPokemonAssets),
      initCommonAnims().then(() => loadCommonAnimAssets(true)),
      Promise.all([Moves.ARK_01, Moves.ARK_02, Moves.ARK_03, Moves.ARK_04].map(m => initMoveAnim(m))).then(
  ...
  reset(clearScene = false, clearData = false, reloadI18n = false): void {
    ...
    this.newArena(Overrides.STARTING_BIOME_OVERRIDE || Biome.Rhodes);
  ...
  generateRandomBiome(waveIndex: number): Biome {
    ...
    const biomes = getEnumValues(Biome).filter(b => b !== Biome.Rhodes && b !== Biome.END);
