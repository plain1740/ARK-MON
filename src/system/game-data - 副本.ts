...
export const defaultStarterSpecies: Species[]  = Object.values(Species)
  .filter(v => typeof v === "number") as Species[];
...
private unlockAllShinyForms(): void {
  for (const speciesIdStr in this.dexData) {
    const entry = this.dexData[speciesIdStr];
    // 由于 DexAttr.SHINY 是 BigInt，使用按位或运算
    entry.seenAttr = entry.seenAttr | DexAttr.SHINY;
    entry.caughtAttr = entry.caughtAttr | DexAttr.SHINY;
  }
}
