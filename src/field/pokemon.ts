...
   */
  async loadAssets(ignoreOverride = true, useIllusion: boolean = false): Promise<void> {
    ...
    if (this.isPlayer() || this.getFusionSpeciesForm(false, useIllusion)) {
      ...
      globalScene.loadPokemonAtlas(
        this.getBattleSpriteKey(true, ignoreOverride,true),
        this.getBattleSpriteAtlasPath(true, ignoreOverride,true),
      );
    }
    ...
    if (this.isPlayer()) {
      ...
      const battleFrameNames_isin = globalScene.anims.generateFrameNames(this.getBattleSpriteKey(this.isPlayer(),true,true), {
        zeroPad: 4,
        suffix: ".png",
        start: 1,
        end: 240,
      ...
      globalScene.anims.create({
        key: this.getBattleSpriteKey(this.isPlayer(),true,true),
        frames: battleFrameNames_isin,
        frameRate: 10,
        repeat: 0,
      });
  ...
  getBattleSpriteAtlasPath(back?: boolean, ignoreOverride?: boolean,isin?=false): string {
    const spriteId = this.getBattleSpriteId(back, ignoreOverride,isin).replace(
      /\_{2}/g,
      "/",
    );
    return `${/_[1-3]$/.test(spriteId) ? "variant/" : ""}${spriteId}`;
  ...
  getBattleSpriteId(back?: boolean, ignoreOverride?: boolean,isin?=false ): string {
    if (back === undefined) {
      back = this.isPlayer();
    }
  ...
  getBattleSpriteKey(back?: boolean, ignoreOverride?: boolean,isin?=false): string {
    return `pkmn__${this.getBattleSpriteId(back, ignoreOverride,isin)}`;
  }
  ...
  playAnim(): void {
    ...
    this.tryPlaySprite(
      this.getSprite(),
      this.getTintSprite()!,
this.getBattleSpriteKey(this.isPlayer(),true,true) ,  this.getBattleSpriteKey()
  ...
  getStatStages(): number[] {
    return this.summonData ? this.summonData.statStages : [0, 0, 0, 0, 0, 0, 0,0];
  }
  ...
  getStatStage(stat: BattleStat): number {
    return this.summonData ? this.summonData.statStages[stat] : 0;
  }
  ...
  setStatStage(stat: BattleStat, value: number): void {
    if (value >= -6) {
      this.summonData.statStages[stat] = Math.min(value, 6);
    } else {
      this.summonData.statStages[stat] = Math.max(value, -6);
    }
  }
...
getMaxHp(): number {
  const baseHp = this.getStat(Stat.HP); // 获取原始HP
  const stage = this.getStatStage(Stat.HP);   // 获取当前阶段值
  ...
  */
  ...
  enableMask() {
    if (!this.maskEnabled) {
      ...
      this.maskSprite?.setPosition(
        ...
        this.y * this.parentContainer.scale + this.parentContainer.y+250,
      );
  ...
  disableMask() {
    if (this.maskEnabled) {
      ...
      this.maskSprite?.setPosition(0, 250);
...
export class PokemonSummonData {
  /** [Atk, Def, SpAtk, SpDef, Spd, Acc, Eva,HP] */
  public statStages: number[] = [0, 0, 0, 0, 0, 0, 0,0 ];
