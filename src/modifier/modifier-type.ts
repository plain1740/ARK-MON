...
const tmmodifierPool: ModifierPool = {
  [ModifierTier.COMMON]: [    new WeightedModifierType(modifierTypes.TM_COMMON, 2),
  ].map(m => {
    m.setTier(ModifierTier.COMMON);
    return m;
  }),
  [ModifierTier.GREAT]: [
    new WeightedModifierType(modifierTypes.TM_GREAT, 2)  ].map(m => {
    m.setTier(ModifierTier.GREAT);
    return m;
 }),
  [ModifierTier.ULTRA]: [
    new WeightedModifierType(modifierTypes.TM_ULTRA, 2),].map(m => {
...
export function regenerateModifierPoolThresholds(party: Pokemon[], poolType: ModifierPoolType, rerollCount = 0) {
  ...
  switch (poolType) {
    ...
    case ModifierPoolType.DAILY_STARTER:
      ...
      break;
    case ModifierPoolType.TM:
      tmmodifierPoolThresholds = thresholds;
      break;
  }
}
...
function getModifierTypeOptionWithRetry(
  ...
  tier?: ModifierTier,
  allowLuckUpgrades?: boolean,
  checkExisting: boolean = true,  // ✅ 新增参数：是否检查已有选项
): ModifierTypeOption {
...
{        
  ...
  while (
    ...
    existingOptions.some(
      o => o.type.name === candidate?.type.name || o.type.group === candidate?.type.group
    )
  ) {
    candidate = getNewModifierTypeOption(
      party,
...
function getNewModifierTypeOption(
  ...
  switch (poolType) {
    ...
    case ModifierPoolType.DAILY_STARTER:
      ...
      break;
    case ModifierPoolType.TM:
      thresholds = tmmodifierPoolThresholds;
      break;
  }
  if (tier === undefined) {
...
pool[tier].forEach((entry, i) => {
  //let type = entry.modifierType;
  //if (type instanceof ModifierTypeGenerator) {
   // type = type.generateType(party);
 // }
  // 如果 type 是 null 直接显示 null，否则显示某个字段
 // console.log(`  [${i}]`, type ? (type.id || type.name || type.key || type.toString()) : null);
});
  }
