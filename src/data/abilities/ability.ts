...
export class PostVictoryHealAbAttr extends PostVictoryAbAttr {
  private ratio: number; // 回复比例，例如 0.5 表示回复最大 HP 的 50%
  ...
  override applyPostVictory(pokemon: Pokemon, passive: boolean, simulated: boolean, args: any[]): void {
    if (!simulated) {
      const healAmount = toDmgValue(pokemon.getMaxHp() * this.ratio);
      pokemon.heal(healAmount);
      pokemon.updateInfo(); // 确保 UI 正确刷新
    }
  }
}
...
export class PostSummonGiveSelfAndAllyTagAbAttr extends PostSummonAbAttr {
  private tagType: BattlerTagType;
  private turnCount: number;
  ...
  override canApplyPostSummon(pokemon: Pokemon, passive: boolean, simulated: boolean, args: any[]): boolean {
    const ally = pokemon.getAlly();
    // 检查自己或队友中至少有一个可以添加 tag
    return pokemon.canAddTag(this.tagType) || (ally?.isActive(true) && ally.canAddTag(this.tagType));
  }
  ...
  override applyPostSummon(pokemon: Pokemon, passive: boolean, simulated: boolean, args: any[]): void {
    ...
    // 给自己加 tag
    if (!simulated && pokemon.canAddTag(this.tagType)) {
      pokemon.addTag(this.tagType, this.turnCount);
    }

    // 给队友加 tag（如果是激活状态）
    if (!simulated && ally?.isActive(true) && ally.canAddTag(this.tagType)) {
      ally.addTag(this.tagType, this.turnCount);
        }
  }
}
...
function createTypeMatchCondition(
  allowedMoveTypes?: PokemonType[],
  allowedDefenderTypes?: PokemonType[]
): PokemonAttackCondition {
  return (user, target, move) => {
    if (!move || !target) return false;

    const moveType = move.type;
    const defenderTypes = target.getTypes?.() ?? [];

    const moveTypeOk =
      !allowedMoveTypes || allowedMoveTypes.includes(moveType);
    const defenderTypeOk =
      !allowedDefenderTypes ||
      defenderTypes.some(type => allowedDefenderTypes.includes(type));

    return moveTypeOk && defenderTypeOk;
  };
}
...
export class MoodyAbAttr extends PostTurnAbAttr {
  constructor(
    private raiseAmount: number = 2,
    private lowerAmount: number = 1
  ) {
    super(true);
  }

  /**
   * Randomly increases one stat stage by raiseAmount and decreases a different stat stage by lowerAmount
   */
  override applyPostTurn(pokemon: Pokemon, passive: boolean, simulated: boolean, args: any[]): void {
    const canRaise = this.raiseAmount > 0
      ? EFFECTIVE_STATS.filter(s => pokemon.getStatStage(s) < 6)
      : [];

    let canLower = this.lowerAmount > 0
      ? EFFECTIVE_STATS.filter(s => pokemon.getStatStage(s) > -6)
      : [];

    if (!simulated) {
      let raisedStat: StatID | null = null;

      if (canRaise.length > 0) {
        raisedStat = canRaise[pokemon.randSeedInt(canRaise.length)];
        canLower = canLower.filter(s => s !== raisedStat);  // 避免重复
        globalScene.unshiftPhase(
          new StatStageChangePhase(pokemon.getBattlerIndex(), true, [raisedStat], this.raiseAmount)
        );
      }
      ...
      if (canLower.length > 0) {
        ...
        globalScene.unshiftPhase(
          new StatStageChangePhase(pokemon.getBattlerIndex(), true, [loweredStat], -this.lowerAmount)
        );
      }
    }
  }
...
export function initAbilities() {
  allAbilities.push(
    ...
    new Ability(Abilities.ARK_A1, 1)
      .attr(PostSummonAllyHealAbAttr, 4, true),
    new Ability(Abilities.ARK_A2, 1)
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.ATK,Stat.DEF ], 1, true),
    new Ability(Abilities.ARK_A3, 1)
      .attr(ReceivedMoveDamageMultiplierAbAttr, (target, user, move) => move.category === MoveCategory.PHYSICAL, 0.5)
      .ignorable(),
    new Ability(Abilities.ARK_A4, 1)
      .attr(DamageBoostAbAttr,2,((user, target, move) => (target?.getMoveEffectiveness(user!, move) ?? 1) <= 0.5 ) &&createTypeMatchCondition([PokemonType.BOW],undefined)),
    new Ability(Abilities.ARK_A5, 1)
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.EVA ], 1, true),
    new Ability(Abilities.ARK_A6, 1)
      .attr(WonderSkinAbAttr)
      .ignorable(),
    new Ability(Abilities.ARK_A7, 1)
      .attr(BonusCritAbAttr),
    new Ability(Abilities.ARK_A8, 1)
      .attr(DamageBoostAbAttr,2,createTypeMatchCondition([PokemonType.CROSSBOW],[PokemonType.SHIELD])),
    new Ability(Abilities.ARK_A9, 1)
     .attr(DamageBoostAbAttr,2,createTypeMatchCondition([PokemonType.A_NEUTRALIZE],[PokemonType.SHIELD])),
    new Ability(Abilities.ARK_A10, 1)
      .attr(FieldMultiplyStatAbAttr, Stat.SPDEF, 0.8)
      .attr(PostSummonMessageAbAttr, (user) => i18next.t("abilityTriggers:postSummonHSMW", { pokemonNameWithAffix: getPokemonNameWithAffix(user), statName: i18next.t(getStatKey(Stat.SPDEF)) })),
    new Ability(Abilities.ARK_A11, 1)
         .attr(MoodyAbAttr,1,0),
    new Ability(Abilities.ARK_A12, 1)
      .attr(PostItemLostApplyBattlerTagAbAttr, BattlerTagType.UNBURDEN)
      .bypassFaint() // Allows reviver seed to activate Unburden
      .edgeCase(), // Should not restore Unburden boost if Pokemon loses then regains Unburden ability
    new Ability(Abilities.ARK_A13, 1)
      .attr(MovePowerBoostAbAttr, (user, target, move) => {
        const movePhase = globalScene.findPhase((phase) => phase instanceof MovePhase && phase.pokemon.id !== user?.id);
        return isNullOrUndefined(movePhase);
      }, 1.3),
    new Ability(Abilities.ARK_A14, 1)
      .attr(AttackTypeImmunityAbAttr, PokemonType.A_ICE)
      .ignorable(),
    new Ability(Abilities.ARK_A15, 1)
      .attr(PostVictoryStatStageChangeAbAttr, Stat.SPD, 1),
    new Ability(Abilities.ARK_A16, 1)
  .conditionalAttr(
    p => !globalScene.currentBattle.double || p.getAlly() === undefined,
    StatMultiplierAbAttr,  Stat.ATK,  1.3  )
  .conditionalAttr(
    p => !globalScene.currentBattle.double || p.getAlly() === undefined,
    StatMultiplierAbAttr,  Stat.DEF,  1.3  ),
    new Ability(Abilities.ARK_A17, 1)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.LANCE, 1.3),
    new Ability(Abilities.ARK_A18, 1)
  .conditionalAttr(
  p => globalScene.currentBattle.double && 
      [Species.DONDOZO, Species.SPECIES_X, Species.SPECIES_Y].includes(p.getAlly()?.species.speciesId ?? -1),
StatMultiplierAbAttr, Stat.ATK, 1.5 )
  .conditionalAttr(
  p => globalScene.currentBattle.double && 
      [Species.DONDOZO, Species.SPECIES_X, Species.SPECIES_Y].includes(p.getAlly()?.species.speciesId ?? -1),
StatMultiplierAbAttr, Stat.SPD, 1.5 ),
    new Ability(Abilities.ARK_A19, 1)
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.HP ], 1, true)
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.DEF ], -1, true),
    new Ability(Abilities.ARK_A20, 1)
      .attr(PostVictoryHealAbAttr,0.25),
    new Ability(Abilities.ARK_A21, 1)
      .attr(MaxMultiHitAbAttr),
    new Ability(Abilities.ARK_A22, 1)     
 .attr(PostSummonGiveSelfAndAllyTagAbAttr, BattlerTagType.AQUA_RING,4),
    new Ability(Abilities.ARK_A23, 1)
      .attr(StatMultiplierAbAttr, Stat.SPDEF, 1.2),
    new Ability(Abilities.ARK_A24, 1)
      .attr(StatMultiplierAbAttr, Stat.DEF, 1.2),
    new Ability(Abilities.ARK_A25, 1)
      .attr(PostAttackContactApplyStatusEffectAbAttr, 30, StatusEffect.PARALYSIS),
    new Ability(Abilities.ARK_A26, 1)
      .attr(AddSecondStrikeAbAttr, 0.2),
    new Ability(Abilities.ARK_A27, 1)
      .attr(StatMultiplierAbAttr, Stat.SPDEF, 1.2),
    new Ability(Abilities.ARK_A28, 1)
      .attr(FieldMultiplyStatAbAttr, Stat.DEF, 0.8)
      .attr(PostSummonMessageAbAttr, (user) => i18next.t("abilityTriggers:rRJ", { pokemonNameWithAffix: getPokemonNameWithAffix(user), statName: i18next.t(getStatKey(Stat.DEF)) })),
    new Ability(Abilities.ARK_A29, 1)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.FIST, 1.3),
    new Ability(Abilities.ARK_A30, 1)
      .attr(PostSummonMessageAbAttr, (pokemon: Pokemon) => i18next.t("abilityTriggers:jSCH", { pokemonNameWithAffix: getPokemonNameWithAffix(pokemon) }))
      .attr(MoveAbilityBypassAbAttr),
    new Ability(Abilities.ARK_A31, 1)
	.attr(PreSwitchOutHealAbAttr),
    new Ability(Abilities.ARK_A32, 1)
      .attr(PostAttackApplyStatusEffectAbAttr, 30, StatusEffect.POISON),
    new Ability(Abilities.ARK_A33, 1)
      .attr(MovePowerBoostAbAttr, (user, target, move) => {
        const movePhase = globalScene.findPhase((phase) => phase instanceof MovePhase && phase.pokemon.id !== user?.id);
        return isNullOrUndefined(movePhase);
      }, 1.3),
    new Ability(Abilities.ARK_A34, 1)
      .attr(AllyMoveCategoryPowerBoostAbAttr, [ MoveCategory.SPECIAL ], 1.3),
    new Ability(Abilities.ARK_A35, 1)
      .attr(AddSecondStrikeAbAttr, 0.2),
    new Ability(Abilities.ARK_A36, 1)
      .attr(ChangeMovePriorityAbAttr, (pokemon, move) => move.hasFlag(MoveFlags.TRIAGE_MOVE), 3),
    new Ability(Abilities.ARK_A37, 1)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.A_HEALING, 1.3),
    new Ability(Abilities.ARK_A38, 1)
      .attr(MovePowerBoostAbAttr, (user, target, move) => {
        const power = new NumberHolder(move.power);
        applyMoveAttrs(VariablePowerAttr, user, target, move, power);
        return power.value <= 60;
      }, 1.5),
    new Ability(Abilities.ARK_A39, 1)
      .attr(PostDefendStatStageChangeAbAttr, (target, user, move) => move.category !== MoveCategory.STATUS, Stat.DEF, 1),
    new Ability(Abilities.ARK_A40, 1)
      .attr(DamageBoostAbAttr,2,
((user, target, move) => (target?.getMoveEffectiveness(user!, move) ?? 1) <= 0.5)&&createTypeMatchCondition(undefined,[PokemonType.BOW,PokemonType.CROSSBOW,PokemonType.SHOTGUN,
PokemonType.CANNON,PokemonType.MUSKET,PokemonType.BOOMERANG])
),
 new Ability(Abilities.ARK_A41, 1)
      .attr(MoveImmunityAbAttr, (pokemon, attacker, move) => pokemon.getAlly() === attacker && move instanceof AttackMove)
      .ignorable(),
 new Ability(Abilities.ARK_A42, 1)
      .attr(MovePowerBoostAbAttr, (user, target, move) => move.chance >= 1, 1.3)
      .attr(MoveEffectChanceMultiplierAbAttr, 0), // This attribute does not seem to function - Should disable life orb, eject button, red card, kee/maranga berry if they get implemented
 new Ability(Abilities.ARK_A43, 1)
            .attr(StatMultiplierAbAttr, Stat.DEF, 0.8)
      .attr(StatMultiplierAbAttr, Stat.SPD, 1.4),
 new Ability(Abilities.ARK_A44, 1)
      .attr(ReceivedMoveDamageMultiplierAbAttr, (target, user, move) => move.category === MoveCategory.PHYSICAL, 0.5)
      .ignorable(),
 new Ability(Abilities.ARK_A45, 1)
      .attr(MoveTypePowerBoostAbAttr, PokemonType.A_FIRE, 1.3),
 new Ability(Abilities.ARK_A46, 1)
      .attr(FieldMultiplyStatAbAttr, Stat.DEF, 1.25)
      .attr(PostSummonMessageAbAttr, (user) => i18next.t("abilityTriggers:hEM", { pokemonNameWithAffix: getPokemonNameWithAffix(user), statName: i18next.t(getStatKey(Stat.DEF)) })),
new Ability(Abilities.ARK_A47, 1)
      .attr(FieldMultiplyStatAbAttr, Stat.SPDEF, 1.25)
      .attr(PostSummonMessageAbAttr, (user) => i18next.t("abilityTriggers:bAM", { pokemonNameWithAffix: getPokemonNameWithAffix(user), statName: i18next.t(getStatKey(Stat.SPDEF)) })),
new Ability(Abilities.ARK_A48, 1)
         .attr(MoodyAbAttr, 1, 0),
new Ability(Abilities.ARK_A49, 1)
      .attr(StatMultiplierAbAttr, Stat.ATK, 1.2),
new Ability(Abilities.ARK_A50, 1)
      .attr(BlockCritAbAttr)
      .ignorable(),
new Ability(Abilities.ARK_A51, 1)
      .attr(MovePowerBoostAbAttr, (user, target, move) => {
        const movePhase = globalScene.findPhase((phase) => phase instanceof MovePhase && phase.pokemon.id !== user?.id);
        return isNullOrUndefined(movePhase);
      }, 1.3),
new Ability(Abilities.ARK_A52, 1)
      .attr(ProtectStatAbAttr)
      .ignorable(),
new Ability(Abilities.ARK_A53, 1)
      .attr(AttackTypeImmunityAbAttr, PokemonType.A_DARK)
      .ignorable(),
new Ability(Abilities.ARK_A54, 1)
      .attr(BattlerTagImmunityAbAttr, [ BattlerTagType.INFATUATED, BattlerTagType.TAUNT ])
      .attr(PostSummonRemoveBattlerTagAbAttr, BattlerTagType.INFATUATED, BattlerTagType.TAUNT)
      .attr(IntimidateImmunityAbAttr)
      .ignorable(),
new Ability(Abilities.ARK_A55, 1)
      .attr(StatusEffectImmunityAbAttr, StatusEffect.PARALYSIS)
      .attr(PostSummonHealStatusAbAttr, StatusEffect.PARALYSIS)
      .ignorable(),
new Ability(Abilities.ARK_A56, 1)
      .attr(DamageBoostAbAttr,1.1,createTypeMatchCondition([PokemonType.SWORD,PokemonType.LANCE,PokemonType.DAGGER,PokemonType.FIST,PokemonType.WHIP,PokemonType.AXE,PokemonType.HAMMER,PokemonType.CHAINSAW,PokemonType.SCYTHE,PokemonType.BOW,PokemonType.CROSSBOW,PokemonType.SHOTGUN,PokemonType.CANNON,PokemonType.MUSKET,PokemonType.BOOMERANG,PokemonType.SHIELD]
,undefined)),
new Ability(Abilities.ARK_A57, 1)
      .attr(RedirectTypeMoveAbAttr, PokemonType.A_WATER)
      .attr(TypeImmunityStatStageChangeAbAttr, PokemonType.A_WATER, Stat.SPATK, 1)
      .ignorable(),
new Ability(Abilities.ARK_A58, 1)
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.ATK ], -1, false, true),
new Ability(Abilities.ARK_A59, 1)
      .conditionalAttr(pokemon => !!pokemon.status, StatMultiplierAbAttr, Stat.SPDEF, 1.5)
      .ignorable(),
new Ability(Abilities.ARK_A60, 1)
      .attr(ReverseDrainAbAttr),
new Ability(Abilities.ARK_A61, 1)
      .attr(ArenaTrapAbAttr, (user, target) => {
        if (target.hasAbility(Abilities.SHADOW_TAG)) {
          return false;
        }
...
new Ability(Abilities.ARK_A62, 1)
      .attr(PostSummonStatStageChangeAbAttr, [ Stat.EVA ], 1, true),
new Ability(Abilities.ARK_A63, 1)
.attr(MoodyAbAttr,2, 1),
new Ability(Abilities.ARK_A64, 1)
      .attr(PostDefendCritStatStageChangeAbAttr, Stat.ATK, 6),
new Ability(Abilities.ARK_A65, 1)
      .attr(MoveEffectChanceMultiplierAbAttr, 2),
new Ability(Abilities.ARK_A66, 1)
      .attr(AllyMoveCategoryPowerBoostAbAttr, [ MoveCategory.SPECIAL ], 1.3),
new Ability(Abilities.ARK_A67, 1)
      .attr(PostDefendContactApplyTagChanceAbAttr, 30, BattlerTagType.INFATUATED),
new Ability(Abilities.ARK_A68, 1)
      .attr(RunSuccessAbAttr),
new Ability(Abilities.ARK_A69, 1)
      .conditionalAttr(pokemon => !!pokemon.status, StatMultiplierAbAttr, Stat.SPD, 1.5)
      .ignorable(),
new Ability(Abilities.ARK_A70, 1)
      .conditionalAttr(pokemon => !!pokemon.status, StatMultiplierAbAttr, Stat.DEF, 1.5)
      .ignorable(),
new Ability(Abilities.ARK_A71, 1)
      .attr(ProtectStatAbAttr, Stat.ACC)
      .attr(DoubleBattleChanceAbAttr)
      .attr(IgnoreOpponentStatStagesAbAttr, [ Stat.EVA ])
      .ignorable(),
new Ability(Abilities.ARK_A72, 1)
      .attr(PostVictoryStatStageChangeAbAttr, Stat.ATK, 1),
new Ability(Abilities.ARK_A73, 1)
      .attr(PostAttackStealHeldItemAbAttr),
new Ability(Abilities.ARK_A74, 1)
      .attr(StatStageChangeMultiplierAbAttr, -1)
      .ignorable()
 );
}
