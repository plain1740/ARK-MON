...
const speciesListA = catchableSpecies[species.speciesId] || [];
const speciesListB = catchableSpecies[starterId] || [];
const biomes = speciesListA.concat(speciesListB).map(b => Biome[b.biome]);
    ...
    if (species) {
      ...
      if (shouldUpdateSprite) {
        species.loadAssets(female!, formIndex, shiny, variant, true).then(() => {
         ...
         this.pokemonSprite.setScale(0.2);
